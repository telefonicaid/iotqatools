...
class CkanUtils(object):
	...
	def get_last_value_from_resource(context, endpoint, apikey, resourceid):
		...
		try:
			context.config['components']['CKAN']['response'] = context.o['CKAN']. \
				get_last_value_from_resource(context,
	...
	@staticmethod
	def ckan_create_org(context, comp, service, secure=False):
		...
		try:
			context.config['components'][comp]['response'] = context.o[comp].ckan_create_organization(service,
																									  verify_ssl=verify_ssl)
	...
	@staticmethod
	def ckan_create_pkg(context, comp, service, secure=False):
		...
		verify_ssl = ast.literal_eval(context.config['components'][comp]['verifyssl'])
		try:
			context.config['components'][comp]['response'] = context.o[comp].ckan_create_pkg(package, service,
																							 verify_ssl=verify_ssl)
		except requests.exceptions.RequestException, e:
	...
	@staticmethod
	def ckan_create_rsc(context, comp, service, secure=False):
		# collect all the input data and mixed it with the template to send
		assert_true(service == service.lower())
		...
		verify_ssl = ast.literal_eval(context.config['components'][comp]['verifyssl'])
		try:
			context.config['components'][comp]['response'] = context.o[comp].ckan_create_rsc(resource_name, package_id,
	...
	@staticmethod
	def ckan_get_rsc(context, comp, service, secure=False):
		# collect all the input data and mixed it with the template to send
		assert_true(service == service.lower())
		...
		verify_ssl = ast.literal_eval(context.config['components'][comp]['verifyssl'])
		try:
			context.config['components'][comp]['response'] = context.o[comp].ckan_get_pkg(package_name,
																						  verify_ssl=verify_ssl)
		except requests.exceptions.RequestException, e:
			context.config['components'][comp]['response'] = 'ERROR'
			__logger__.info("Network error: " + e)
	...
	@staticmethod
	def ckan_get_pkg(context, comp, service, secure=False):
		# collect all the input data and mixed it with the template to send
		assert_true(service == service.lower())
		data = PqaTools.pattern_recall('CKAN', 'show_package')
		package_name = json.loads(data)['id']
	...
	@staticmethod
	def ckan_create_ds(context, comp, service, secure=False):
		# collect all the input data and mixed it with the template to send
		assert_true(service == service.lower())
		...
		verify_ssl = ast.literal_eval(context.config['components'][comp]['verifyssl'])
		try:
			context.config['components'][comp]['response'] = context.o[comp].ckan_create_ds(
				context.remember['resource'], context.remember['package'],
	...
	@staticmethod
	def ckan_create_ds2(context, comp, service, secure=False):
		# collect all the input data and mixed it with the template to send
		assert_true(service == service.lower())
		...
		verify_ssl = ast.literal_eval(context.config['components'][comp]['verifyssl'])
		try:
			context.config['components'][comp]['response'] = context.o[comp].ckan_create_ds2(
				context.remember['resource'], context.remember['package'],
				fields, verify_ssl=verify_ssl)
		except requests.exceptions.RequestException, e:
			context.config['components'][comp]['response'] = 'ERROR'
	...
	@staticmethod
	def ckan_delete_org(context, comp, service, secure=False):
		# collect all the input data and mixed it with the template to send
		assert_true(service == service.lower())
		# verifySSL = False
		verify_ssl = ast.literal_eval(context.config['components']['CKAN']['verifyssl'])
		try:
			context.config['components'][comp]['response'] = context.o['CKAN'].ckan_delete_org(service,
																							   verify_ssl=verify_ssl)
